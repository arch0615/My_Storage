...
-- RLS: Any authenticated user can view all clients
DO $$
BEGIN
  CREATE POLICY "Authenticated users can view all clients" ON public.clients FOR SELECT USING (auth.uid() IS NOT NULL);
...
DO $$
BEGIN
  CREATE POLICY "Authenticated users can update all clients" ON public.clients FOR UPDATE USING (auth.uid() IS NOT NULL);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
DO $$
BEGIN
  CREATE POLICY "Authenticated users can delete all clients" ON public.clients FOR DELETE USING (auth.uid() IS NOT NULL);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
